...
public class IRGenerator implements IRTraversal {
    ...
    public IRGenerator(ProgNode progNode) {
        program = progNode;
...
//        VirtualRegister dest = currentFunction.getRegister();
        ...
        VirtualRegister dest = curFuncScope.getRegister();
    ...
    public IntegerValue visit(PrefixExprNode node) {
        ...
        switch (node.getOption()) {
            case NEG:
                addInst(new Alloca(curLab, address, new BuiltIn()));
                curFuncScope.incSlotNum();
                addInst(new grahamcompiler.IR.BinaryOp(curLab, grahamcompiler.IR.BinaryOp.BinOp.neg, address,
                        value,null));
                return address;
            case NOT:
                addInst(new Alloca(curLab, address, new BuiltIn()));
                curFuncScope.incSlotNum();
                addInst(new grahamcompiler.IR.BinaryOp(curLab, grahamcompiler.IR.BinaryOp.BinOp.xor, address,
                        value, new Immediate(1)));
                return address;
            case BIT_NOT:
                addInst(new Alloca(curLab, address, new BuiltIn()));
                curFuncScope.incSlotNum();
                addInst(new grahamcompiler.IR.BinaryOp(curLab, grahamcompiler.IR.BinaryOp.BinOp.not, address,
                        value, null));
                return address;
            case POS:
                return value;
            case DEC:
                addInst(new grahamcompiler.IR.BinaryOp(curLab, grahamcompiler.IR.BinaryOp.BinOp.sub, (Address) value,
                        value, new Immediate(1)));
                addInst(new Store(curLab, value, register));
                return value;
            case INC:
                addInst(new grahamcompiler.IR.BinaryOp(curLab, grahamcompiler.IR.BinaryOp.BinOp.add, (Address) value,
                        value, new Immediate(1)));
                addInst(new Store(curLab, value, register));
                return value;
        }
//        return register;
        throw new RuntimeException("Prefix at the wrong place");
    }
    ...
    public IntegerValue visit(SuffixExprNode node) {
        IntegerValue value = visit(node.getExpr());
        VirtualRegister new_reg = curFuncScope.getRegister();
        VirtualRegister origin_reg = curFuncScope.getRegister();
        IRType irType;
        irType = new BuiltIn();
        ...
        switch(node.getOption()) {
            ...
            case SUF_INC:
                addInst(new grahamcompiler.IR.BinaryOp(curLab, grahamcompiler.IR.BinaryOp.BinOp.add,
                        (Address) value, value, new Immediate(1)));
                return origin_address;
        }
        throw new RuntimeException("Suffix at the wrong place");
    }
    ...
    @Override
    public IRInstruction visit(ExprStmtNode node) {
        if(node == null) return null;
    ...
    private void addBinaryInst(Address dest, IntegerValue left, IntegerValue right, BinaryOp op) {
        if(BinaryOp.isCompare(op)) {
            ...
            grahamcompiler.IR.BinaryOp.BinOp binaryOp;
            switch (op) {
                case ADD: binaryOp = grahamcompiler.IR.BinaryOp.BinOp.add; break;
                case MIN: binaryOp = grahamcompiler.IR.BinaryOp.BinOp.sub; break;
                case MUL: binaryOp = grahamcompiler.IR.BinaryOp.BinOp.imul; break;
                case DIV: binaryOp = grahamcompiler.IR.BinaryOp.BinOp.idiv; break;
                case L_SHIFT: binaryOp = grahamcompiler.IR.BinaryOp.BinOp.sal; break;
                case R_SHIFT: binaryOp = grahamcompiler.IR.BinaryOp.BinOp.sar; break;
                case MOD: binaryOp = grahamcompiler.IR.BinaryOp.BinOp.mod; break;
                case BIT_AND: binaryOp = grahamcompiler.IR.BinaryOp.BinOp.and; break;
                case BIT_OR: binaryOp = grahamcompiler.IR.BinaryOp.BinOp.or; break;
                case BIT_XOR: binaryOp = grahamcompiler.IR.BinaryOp.BinOp.xor; break;
                default: binaryOp = null; break;
            }
            addInst(new grahamcompiler.IR.BinaryOp(curLab, binaryOp, dest, left, right));
        }
    }
    ...
    private Address memoryAllocate(List<ExprNode> node, Type type, boolean isTop, Address address) {
        ...
        if (node.size() != 0) {
            ...
            curFuncScope.incSlotNum();
            addInst(new grahamcompiler.IR.BinaryOp(curLab, grahamcompiler.IR.BinaryOp.BinOp.add, offset, offset, new Immediate(1)));
    ...
    private IntegerValue visitNotCondition(PrefixExprNode node) {
        ...
        addInst(new Alloca(curLab, address, new BuiltIn()));
        addInst(new grahamcompiler.IR.BinaryOp(curLab, grahamcompiler.IR.BinaryOp.BinOp.xor, address, address, new Immediate(1)));
        return address;
    }
