...
public class Semantic implements ASTVisitor {
    ...
    public Semantic(Scope topScope, ERROR handler) {
        error = handler;
    }
    ...
    public void visit(ProgNode node) {
        ...
        if(!currentScope.containsNode(Name.getName("main")) ||
            ...
            error.addError(new location(0,0), "no main function");
    ...
    public void visit(VarDeclNode node) {
        ...
        if(!(currentScope.isClass())) {
            ...
            try {
                ...
                error.addError(node.getLocation(), e.getMessage());
            }
        ...
        if(node.getValue().getExprType().getTypeName() != Name.getName("null")) {
            ...
            if(node.getType() instanceof BuiltInType)
                error.addError(node.getLocation(), "null cannot be assigned to built in type");
        }
    }
    ...
    public void visit(ArrayExprNode node) {
        ...
        if(node.getIndex().getExprType().getTypeName() != Name.getName("int"))
            error.addError(node.getLocation(), "array index must be an integer"
    ...
    public void visit(MemberExprNode node) {
        ...
        if(!(node.getExpress().getExprType() instanceof ClassType)) {
            if(node.getExpress().getExprType() instanceof ArrayType
                ...
                error.addError(node.getLocation(), "wrong usage of '.'");
            }
            return;
        }
        ...
        if(!scope.containsNode(node.getName())) {
            error.addError(node.getLocation(), node.getName().toString() + " have not been declared");
        }
    ...
    public void visit(ThisExprNode node) {
        if(currentClass == null)
            error.addError(node.getLocation(), "'this' should be in a class");
    ...
    public void visit(BreakNode node) {
        if(!currentScope.isLoop())
            error.addError(node.getLocation(), "'break' must be in a loop");
    }

    @Override
    public void visit(ContinueNode node) {
        if(!currentScope.isLoop())
            error.addError(node.getLocation(), "'continue' must be in a loop");
    }
    ...
    public void visit(ReturnNode node) {
        ...
        if(!currentScope.isFunction())
            error.addError(node.getLocation(), "'return' must be in a function");
        if(node.getExpress() == null) return;
        if(currentFunction.isConstructor() && node.getExpress() != null) {
            error.addError(node.getLocation(), "cannot return any value in constructors");
        }
        visit(node.getExpress());
    ...
    private void checkParameterMatch(FuncDeclNode decl, CallExprNode call) {
        if(decl.getParameter().size() != call.getParameter().getExpresses().size())
            error.addError(call.getLocation(),
